...
RUN node scripts/generate-gemini.mjs && chmod -R a=rX /app/build/gemini
...
COPY --from=capsule /app/build/gemini/ /srv/capsule/
COPY --from=cv-pdf /cv/out/ /srv/capsule/cv/
